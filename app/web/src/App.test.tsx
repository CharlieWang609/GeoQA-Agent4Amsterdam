...
describe("Live Sandbox", () => {
  beforeEach(() => {
    ...
    vi.mocked(getObservations).mockResolvedValue({ items: [] });
    vi.mocked(getModels).mockResolvedValue({
      default: "gemini-3.8-flash",
      models: [
        { model: "gemini-3.8-flash", label: "Gemini 3.8 Flash", provider: "google" },
  ...
  it("opens as a two-pane governed workspace with the contractual controls", async () => {
    render(<App />);
  ...
  it("expands truncated example questions on hover or focus and collapses afterward", () => {
    ...
    const exampleQuestion = screen.getByRole("button", {
      name: "Which Amsterdam neighborhoods have no registered public sports locations?",
    });
  ...
  it("lists owned history, restores a selection, and New Question clears the URL", async () => {
    ...
    vi.mocked(getSession).mockResolvedValue(sessionWith());
    const user = userEvent.setup();
    render(<App />);
  ...
  it("surfaces a rejected deletion without leaving the session", async () => {
    window.history.replaceState({}, "", "/sandbox?session=session-123");
    vi.mocked(getSession).mockResolvedValue(sessionWith());
  ...
  it("keeps the Analysis Plan and all review subsections collapsed with minimal Retrieved Data", async () => {
    ...
    expect([...retrieved.querySelectorAll("dt")].map((item) => item.textContent)).toEqual([
      "Dataset", "Layer", "Geometry", "Attributes",
      "Dataset", "Layer", "Geometry", "Attributes",
    ]);
    expect(within(retrieved).getByText("gebieden")).toBeVisible();
    expect(within(retrieved).getByText("buurten")).toBeVisible();
    expect(within(retrieved).getByText("Polygon")).toBeVisible();
    expect(within(retrieved).getByText("Point")).toBeVisible();
    ...
    expect(within(planDetails).queryByText("Workflow steps")).not.toBeInTheDocument();
    ...
    expect(within(workflowDialog).getByText("Count", { selector: "tspan" })).toBeVisible();
    expect(within(workflowDialog).getByText("countpointsinpolygon", { selector: "tspan" })).toBeVisible();
  ...
  it("keeps plan mutations gated while a result awaits review and after either decision", async () => {
    ...
    vi.mocked(getSession).mockResolvedValue(awaitingDecision);
    const user = userEvent.setup();
    ...
    vi.mocked(getSession).mockResolvedValue(accepted);
    render(<App />);
  ...
  it("keeps source limitation visible and makes the exact result table reachable", async () => {
    window.history.replaceState({}, "", "/sandbox?session=session-123");
    vi.mocked(getSession).mockResolvedValue(candidateSession());
    const user = userEvent.setup();
    render(<App />);
  ...
  it.each(["accepted", "rejected"] as const)(
    ...
    async (decision) => {
      ...
      vi.mocked(decideResult).mockResolvedValue(decided);
      const user = userEvent.setup();
      render(<App />);
      ...
      expect(decideResult).toHaveBeenCalledWith("session-123", decision, null);
  ...
  it("keeps candidate-construction failures visible", async () => {
    window.history.replaceState({}, "", "/sandbox?session=session-123");
    vi.mocked(getSession).mockResolvedValue(sanityCheckFailureSession());
    render(<App />);
