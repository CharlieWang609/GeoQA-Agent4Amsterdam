...
class CatalogLayerPreviewUnavailableError(ValueError):
    """The pinned snapshot cannot be rendered as a layer preview."""
...
def build_catalog_layer_preview(
    ...
    if layer.vector is None:
        raise CatalogLayerPreviewUnavailableError("raster layers have no preview")
    ...
    try:
        ...
        project = display_transformer(layer.crs).transform
...
def _layer_listing(layer: CatalogLayer) -> dict[str, object]:
    ...
    dataset_title_en = (
        None
        if enriched is None or enriched.dataset_title_en is None
        else _resolved(enriched.dataset_title_en)
    )
    dataset_description_en = (
        None
        if enriched is None or enriched.dataset_description_en is None
        else _resolved(enriched.dataset_description_en)
    )
    return {
        "dataset": layer.dataset_id,
        "dataset_title": dataset_title_en or layer.raw.dataset_title,
        "dataset_title_language": (
            "en" if dataset_title_en is not None else "nl"
        ),
        "dataset_description": (
            dataset_description_en or layer.raw.dataset_description
        ),
        "dataset_description_language": (
            "en" if dataset_description_en is not None else "nl"
        ),
        ...
        "bands": [band.name for band in layer.attributes] if layer.raster else [],
...
@lru_cache(maxsize=8)
def display_transformer(source_crs: str) -> Transformer:
    """Cached projection from a stored CRS to browser display coordinates."""
