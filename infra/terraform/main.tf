...
resource "azurerm_container_app" "web" {
  ...
  secret {
    name                = "google-api-key"
    identity            = azurerm_user_assigned_identity.dispatch.id
    key_vault_secret_id = var.google_api_key_secret_id
  }

  secret {
    name                = "deepseek-api-key"
    identity            = azurerm_user_assigned_identity.dispatch.id
    key_vault_secret_id = var.deepseek_api_key_secret_id
  }

  secret {
    name                = "gee-service-account-key"
    identity            = azurerm_user_assigned_identity.dispatch.id
    key_vault_secret_id = var.gee_service_account_key_secret_id
  }
  ...
  template {
    ...
    container {
      ...
      # The Earth Engine key JSON itself; the runner initialises from it
      # without writing it to disk.
      env {
        name        = "GEE_SERVICE_ACCOUNT_KEY"
        secret_name = "gee-service-account-key"
      }

      # Planning and execution run in worker threads, but GeoPandas steps
      # hold the GIL in bursts; a 1-second probe timeout killed healthy
      # containers mid-request (2026-09-07).
      liveness_probe {
        transport               = "HTTP"
        port                    = 8000
        path                    = "/api/health"
...
# AllowAnonymous exposes Catalog inspection and the worked examples; the API
# enforces auth per route.
