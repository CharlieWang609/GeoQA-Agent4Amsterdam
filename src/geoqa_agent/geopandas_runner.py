...
def _count_points_in_polygon(parameters: Mapping[str, object]) -> str:
    ...
    # Non-point objects are located at their centroid (a point's centroid
    # is itself), so lines and polygons count in exactly one support.
    joined = gpd.sjoin(
        points.set_geometry(points.geometry.centroid),
    ...
    if parameters.get("statistic", "count") == "sum":
        merged = grouped[str(parameters["value_field"])].sum()
    elif class_field is None:
        merged = grouped.size()
    else:
        merged = grouped[str(class_field)].nunique()
    result = polygons.copy()
    # Every polygon keeps a row: absent groups become explicit zeros.
    result[str(parameters["field"])] = merged.reindex(polygons.index, fill_value=0)
    rows = _write(parameters, "output", result)
