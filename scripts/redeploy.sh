...
print_rollback_guidance() {
  ...
  printf '%s\n' \
    'Rollback-pair rule: "The Catalog pointer and application revision are one rollback unit. Never repoint the Catalog without reverting the web image, and never revert the workload while leaving the newer Catalog pointer active."' >&2
  printf '%s\n' \
